  ...
  try {
    ...
    if (data.success) {
      ...
      data.items.forEach(item => {
        // Parse image_url nếu là mảng JSON
        let imgSrc = 'https://via.placeholder.com/40x40?text=WBS';
        try {
          if (item.image_url) {
            if (item.image_url.startsWith('[')) {
              const arr = JSON.parse(item.image_url);
              if (arr.length > 0) imgSrc = arr[0];
            } else {
        ...
        itemEl.innerHTML = `
          <div style="display: flex; gap: 10px; align-items: center;">
            <img src="${imgSrc}" style="width: 40px; height: 40px; object-fit: cover; border-radius: 4px;"
              onerror="this.src='https://via.placeholder.com/40x40?text=WBS'">
            <div>
